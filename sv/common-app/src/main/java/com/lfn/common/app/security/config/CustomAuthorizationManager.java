...
public final class CustomAuthorizationManager implements AuthorizationManager<RequestAuthorizationContext> {
    ...
    public AuthorizationDecision check(Supplier<Authentication> authentication,
        ...
        if (authorizationContext.getRequest().getUserPrincipal() == null) {
            log.warn("AUTHORIZATION FAILED - Request unauthenticated. URI: {}, Method: {}",
                    authorizationContext.getRequest().getRequestURI(),
                    authorizationContext.getRequest().getMethod());
        ...
        if (authorizationContext.getRequest().getUserPrincipal().getName().equals("anonymousUser")) {
            log.warn("AUTHORIZATION FAILED - Anonymous user attempted access. URI: {}, Method: {}",
                    authorizationContext.getRequest().getRequestURI(),
                    authorizationContext.getRequest().getMethod());
            return new AuthorizationDecision(false);
        }
        ...
        if (HeadersUtil.getAuthorizationToken(authorizationContext.getRequest())!= null) {
            ...
            if (userProjectRoleService
                ...
                log.warn("AUTHORIZATION FAILED - Token expired or invalid. User: {}, URI: {}, Method: {}",
                        authorizationContext.getRequest().getUserPrincipal().getName(),
                        authorizationContext.getRequest().getRequestURI(),
                        authorizationContext.getRequest().getMethod());
                return new AuthorizationDecision(false);
            }
        ...
        if (projectId == null) {
            log.warn("AUTHORIZATION FAILED - Missing Project ID. User: {}, URI: {}, Method: {}",
                    authorizationContext.getRequest().getUserPrincipal().getName(),
                    authorizationContext.getRequest().getRequestURI(),
                    authorizationContext.getRequest().getMethod());
            return new AuthorizationDecision(false);
        ...
        if (roleIdList.isEmpty()) {
            log.warn("AUTHORIZATION FAILED - User has no role mapping for project. User: {}, ProjectId: {}, URI: {}, Method: {}",
                    authorizationContext.getRequest().getUserPrincipal().getName(),
                    projectId,
                    authorizationContext.getRequest().getRequestURI(),
                    authorizationContext.getRequest().getMethod());
        ...
        log.debug("Checking permissions for User: {}, ProjectId: {}, RoleIds: {}, URI: {}, Method: {}",
                authorizationContext.getRequest().getUserPrincipal().getName(),
                projectId, roleIdList,
                authorizationContext.getRequest().getRequestURI(),
                authorizationContext.getRequest().getMethod());
        ...
        for (Integer loop_roleid : roleIdList) {
            for (Map.Entry<String, List<String>> apiMap : this.configurationApisService.getRoleMappedApis(loop_roleid).entrySet()) {
                if (RegularExpressionUtil.matchInputForRegex(urlForValidation,apiMap.getKey())
                    ...
                    log.info("AUTHORIZATION GRANTED - User: {}, ProjectId: {}, RoleId: {}, URI: {}, Method: {}",
                            authorizationContext.getRequest().getUserPrincipal().getName(),
                            projectId, loop_roleid,
                            authorizationContext.getRequest().getRequestURI(),
                            authorizationContext.getRequest().getMethod());
                    return new AuthorizationDecision(true);
                }
        ...
        log.warn("AUTHORIZATION FAILED - No matching permission found. User: {}, ProjectId: {}, RoleIds: {}, URI: {}, Method: {}",
                authorizationContext.getRequest().getUserPrincipal().getName(),
                projectId, roleIdList,
                authorizationContext.getRequest().getRequestURI(),
                authorizationContext.getRequest().getMethod());
