...
export class LitellmComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(private sanitizer: DomSanitizer) {
    this.litellmUrl = environment.litellmUrl?.startsWith('__FE_') ? '/litellm/' : environment.litellmUrl;
  }

  ngOnInit(): void {
    // Guard against unsubstituted build-time placeholders (e.g. __FE_LITELLM_URL__).
    this.isUrlConfigured = !!(this.litellmUrl?.startsWith('http') || this.litellmUrl?.startsWith('/'));
  ...
  private postToIframe(): void {
    ...
    // Resolve against the current location so relative URLs (e.g. '/litellm/') yield a valid origin
    const childOrigin = (() => {
