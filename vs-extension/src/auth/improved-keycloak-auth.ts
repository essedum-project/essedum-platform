...
export class ImprovedKeycloakAuthService {
    ...
    constructor(config: KeycloakConfig, context: vscode.ExtensionContext) {
        ...
        console.log('ImprovedKeycloakAuthService initialized with SSL bypass enabled');
    }
    ...
    private async storeTokens(tokens: TokenResponse): Promise<void> {
        ...
        await this.context.secrets.store(ImprovedKeycloakAuthService.TOKEN_KEY, JSON.stringify(tokenData));
    ...
    public async getStoredTokens(): Promise<TokenResponse | null> {
        try {
            const tokenData = await this.context.secrets.get(ImprovedKeycloakAuthService.TOKEN_KEY);
            if (tokenData) {
    ...
    public async clearStoredTokens(): Promise<void> {
        await this.context.secrets.delete(ImprovedKeycloakAuthService.TOKEN_KEY);
