...
export class InstanceDescriptionComponent {
  ...
  ngOnInit() {
    ...
    this.adapterServices.getInstanceByNameAndOrganization(this.route.snapshot.params.instance).subscribe((resp) => {
      this.card = resp;
  ...
  fetchRelatedConnections(card: any) {
    this.relatedConnections.push(new OptionsDTO('Connection',card.connectionname))
  }
  ...
  deleteInstance(instanceName: string) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === "delete") {
        this.adapterServices.deleteInstance(instanceName).subscribe((res) => {
          if (res) {
  ...
  fethchRelatedInstances(adapterName) {
    this.adapterServices.getMlInstanceNamesByAdapterNameAndOrganization(adapterName).subscribe((resp) => {
      ...
      this.adapterServices.getMlInstanceNamesByOrganization().subscribe((respMlIns: any) => {
        respMlIns.forEach((insName) => {
          if (!this.relatedInstancesList.includes(insName) && insName != this.card.name)
  ...
  fethchAllSpecTemplateNamesByOrg() {
    this.adapterServices.getSpecTemplateNamesByOrganization().subscribe((respSpec) => {
      if (respSpec) {
  ...
  fethchAllAdapterNamesByOrg() {
    this.adapterServices.getAdapterNamesByOrganization().subscribe((respAdp) => {
      if (respAdp) {
        respAdp.forEach(adpName => {
          if (adpName === this.card.adaptername) {
            this.relatedAdapters.push(new OptionsDTO('Implementation', adpName));
  ...
  redirectToSelectedComponent(component: any) {
    if (component.viewValue == 'Instance')
      ...
      this.router.navigate(["../../connections/view/" + this.card.connectionid,true], { relativeTo: this.route })
    }
    else
