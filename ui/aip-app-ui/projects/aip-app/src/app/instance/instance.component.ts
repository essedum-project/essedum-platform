...
export class InstanceComponent implements OnInit, OnChanges{
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (params['page']) {
        ...
        this.selectedConnectionNamesList = params['connectionsList']
          ...
          : [];
        if ((this.selectedAdapterList && this.selectedAdapterList.length > 0) ||
          (this.selectedConnectionNamesList && this.selectedConnectionNamesList.length > 0)) {
  ...
  getCards(page?:any,size?:any): void {
    ...
    this.adapterServices.getInstances(sessionStorage.getItem("organization")).subscribe((res) => {
      let data:any=[];
      let test = res;
      test.forEach((element: any) => {
        ...
        let jobid = element.jobid;
          ...
          this.service.getStatus(jobid).subscribe((res) => {
            ...
            this.adapterServices.updateInstance(element).subscribe(res=>{
  ...
  filterSelectedCards(page: any, size?: any) {
    this.tagrefresh = false;
    if (this.selectedConnectionNamesList.length > 0 || this.selectedAdapterList.length > 0) {
      let data: any = [];
  ...
  tagSelectedEvent(event){
    this.pageNumber=1;
     this.selectedConnectionNamesList=event.getSelectedMlInstanceConnectionType();
  ...
  runScript(data) {
    ...
    this.service.runPipeline(this.streamItem.alias ? this.streamItem.alias : this.streamItem.name, this.streamItem.name, passType, 'REMOTE', data.runtimename, "generated").subscribe(
      res => {
        ...
        this.service.getStreamingServicesByName(this.chainName, data.organization).subscribe(
          (response) => {
