 ...
 connectionPromise:Promise<boolean>;
  ...
  ngOnInit(): void {
   if(this.data && this.data.adaptername){
    this.adapterServices.getAdapteByNameAndOrganization(this.data.adaptername).subscribe(resp=>{
  ...
  authentications() {
    this.service.getPermission("cip").subscribe(
      (cipAuthority) => {
        ...
        else if (!this.data || this.routeToHome)
  ...
  createInstance() {
    ...
    this.adapterServices.createInstance(this.data)
      .subscribe(res => {
  ...
  updateInstance() {
    ...
    this.adapterServices.updateInstance(this.data)
      .subscribe(res => {
  ...
  connectionNameSelectChange(connectionNameSelectd: string) {
    ...
    if (this.selectedConnection.type == 'REST')
      ...
      this.data.executiontype = 'REMOTE';
  }
    ...
    this.adapterServices.getDatasources()
      .subscribe(res => {
        ...
        if (this.action == 'create') {
          this.runtimesForConnection=this.datasourcesForConnection.filter(datasource => datasource.organization == sessionStorage.getItem("organization") && (datasource.type != 'REST') && (datasource.forruntime == '1'));
          this.datasourcesForConnection = this.datasourcesForConnection.filter(datasource => (datasource.interfacetype == null || datasource.interfacetype != 'adapter') && datasource.organization == sessionStorage.getItem("organization") && (datasource.foradapter == '1'));
        } else {
          if (this.data.executiontype == 'REMOTE')
            this.datasourcesForConnection = this.datasourcesForConnection.filter(datasource => (datasource.interfacetype == null && datasource.interfacetype != 'adapter') && datasource.organization == sessionStorage.getItem("organization") && (datasource.type != 'REST') && (datasource.foradapter == '1'));
         else{
            this.runtimesForConnection=this.datasourcesForConnection.filter(datasource => datasource.organization == sessionStorage.getItem("organization") && (datasource.type != 'REST') && (datasource.forruntime == '1'));
            this.datasourcesForConnection = this.datasourcesForConnection.filter(datasource => (datasource.interfacetype == null || datasource.interfacetype != 'adapter') && datasource.organization == sessionStorage.getItem("organization") && (datasource.type == 'REST') && (datasource.foradapter == '1'));
          }
          ...
          this.connectionOptions.push(new OptionsDTO((datasource.type=='REST' ?'REST':'REMOTE').concat('-' + datasource.alias), datasource.alias));
          
        });
        this.runtimesForConnection.forEach((runtime) => {
          this.runtimeOptions.push(new OptionsDTO('REMOTE'.concat('-'+runtime.alias),runtime.name))
          
        });
        if(this.data.runtimename){
          
          this.runtimeAlias = this.runtimesForConnection.filter(datasource=> datasource.name == this.data.runtimename)[0].alias;
  ...
  findAllInstances() {
    this.adapterServices.getInstances(this.org)
      .subscribe(res => {
  ...
  runScript() {
    ...
    // passType = this.type
    this.service.runPipeline(this.streamItem.alias ? this.streamItem.alias : this.streamItem.name, this.streamItem.name, passType, 'REMOTE', this.data.runtimename, "generated").subscribe(
      res => {
        ...
        this.updateInstance();
        this.service.getStreamingServicesByName(this.chainName, this.data.organization).subscribe(
          (response) => {
