import { Component, EventEmitter, Inject, Input, OnDestroy, OnInit, Optional, Output, SkipSelf, ViewChild } from '@angular/core';
...
export class CreateAppComponent implements OnInit, OnDestroy {
  ...
  tags: any = "";
  editImgId: any
  appTypeList = [{ viewValue: "Script", value: "internal" }, { viewValue: "External link", value: "external" },{ viewValue: "MFE", value: "MFE" }, { viewValue: "Embedded link", value: "embed" }, { viewValue: "Chain", value: "chain" }, { viewValue: "Pipeline", value: "pipeline" }, { viewValue: "Video", value: "video" },
    { viewValue: "App", value: "Apps" }
  ];
  typeApp:string='Apps';
  ...
  getVideoDatasets() {
    ...
    this.Services.getVideoDatasets(params).subscribe((res) => {
      res.forEach((element: any) => {
        this.videoDatasetList.push({ viewValue: element.alias, value: element.name });
    });
  });
}
  ...
  saveDetails() {
    if (this.apptype == undefined || this.alias == undefined || this.alias == '' || (this.jobName==undefined && (this.apptype=='video' || this.apptype=='pipeline' || this.apptype=='chain'))){
      this.Services.messageService("Please fill all the mandatory details", "error")
    }
    else {
      try {
        ...
        this.Services.create(newCanvas).subscribe((data: StreamingServices) => {
          data.json_content = JSON.stringify({ "elements": [{ "attributes": { "filetype": "Python3", "files": [data.name + "_" + sessionStorage.getItem('organization') + ".py"], "arguments": [{ name: "App", value: data.alias }], "dataset": [] } }] })
          this.Services.update(data).subscribe();

          this.Services.saveImage(this.alias, data.name, this.chunkMetadata['FileName'], 'image/png/jpg', this.finalImage).subscribe((resp) => {
  ...
  editDetails() {
    ...
    this.Services.getStreamingServicesByName(this.appName).subscribe(resp => {
      let newCanvas = { ...resp };
      newCanvas.alias = this.editCanvas['alias']
      newCanvas.description = this.editCanvas['description']

      const temp = [];
      this.Services.update(newCanvas).subscribe((response) => {
        //this.Services.messageService(response, 'Updated Successfully');
        this.Services.updateImage(this.editImgId, this.editCanvas['alias'], this.appName, this.chunkMetadata['FileName'], 'image/png/jpg', this.finalImage).subscribe((resp) => {
        });
  ...
  setMfeApp(mfeapp: string) {
    this.appMfe = mfeapp
  }

  setAppType(apptype: string) {
    this.apptype = apptype
    if (apptype === 'MFE')  this.isMFEApp = true;
    if (apptype === 'Apps') { this.isApp = true }
    else { this.isApp = false }
    if (apptype === 'chain') {
      this.isChain = true;
    }
    else {
  ...
  getPipelines() {
    ...
    this.Services.getPipelinesByInterfacetype(org, 'pipeline').subscribe((res) => {
      res.forEach((element: any) => {
        if(element.type !='Apps'){
        this.pipelineList.push({ viewValue: element.alias, value: element.name });
        }
      });
    })
  }
  ...
  getPipelinesApps(){
    ...
    this.Services.getPipelinesByInterfacetype(org, 'pipeline').subscribe((res) => {
      res.forEach((element: any) => {
        if(element.type=='Apps'){
          this.pipelineListApps.push({ viewValue: element.alias, value: element.name });
        }
...
uploadFile() {
  try{
    this.Services.uploadToStorageServer(this.storageAttributes).subscribe((response) => {
