...
export class SwaggerCustomComponent implements OnInit {
  ...
  setSwaggerForEndpointByConnectionId() {
    ...
    this.mlspecTemplate = this.mlspecTemplate.replaceAll("{datasource}", this.adapterName).replaceAll("{org}", sessionStorage.getItem("organization")).replace("server_host_url", this.serverUrl);
    this.mlspecTemplate = this.mlspecTemplate.replaceAll("endpoint__id", this.endpointId);
    this.apispecTemplate = JSON.parse(this.mlspecTemplate);
    ...
    if (!this.serverUrls.some(item => item.value === this.apiSpecServerUrl))
      this.serverUrls.push(new OptionsDTO(this.apiSpecServerUrl, this.apiSpecServerUrl));
    this.getParameters();
    this.getPaths();
    this.getSchemas();
      }
  ...
  saveSpecTemplateToEndpoint(body?: any){
    ...
    this.service.updateEndpoint(this.updateEndpoint).subscribe((resp) => {
  ...
  getAdapter() {
    this.adapterServices.getAdapteByNameAndOrganization(this.adapterName).subscribe((res) => {
      this.adapter = res;
      this.adapter.apispec=this.adapter.apispec.replaceAll("{org}", sessionStorage.getItem("organization"));
  ...
  getAdapterInstance() {
    this.adapterServices.getAdapteByNameAndOrganization(this.adapterName).subscribe((res) => {
      this.adapter = res;
      if (this.isInstance)
        this.adapter.apispec = this.adapter.apispec.replaceAll(this.adapterName, this.instanceName);
      this.adapter.apispec = this.adapter.apispec.replaceAll("{org}", sessionStorage.getItem("organization"));
      this.adapterApispecTemplate = JSON.parse(this.adapter.apispec);
      this.getSpecTemplate(this.adapter.spectemplatedomainname);
    });
  ...
  getSpecTemplate(spectemplatedomainname: string) {
    this.adapterServices.fetchApiSpecTemplate(spectemplatedomainname, sessionStorage.getItem("organization")).subscribe((res) => {
      this.mlspecTemplate = res;
      if (!this.isInstance)
        this.mlspecTemplate.apispectemplate = this.mlspecTemplate.apispectemplate.replaceAll("{datasource}", this.adapter.name).replaceAll("{spec}", this.adapter.name).replaceAll("{host}", this.serverUrl).replaceAll("{org}", sessionStorage.getItem("organization"));
      else{
        this.mlspecTemplate.apispectemplate = this.mlspecTemplate.apispectemplate.replaceAll("{datasource}", this.instanceName).replaceAll("{spec}", this.instanceName).replaceAll("{host}", this.serverUrl).replaceAll("{org}", sessionStorage.getItem("organization"));
        this.mlspecTemplate.apispectemplate = this.mlspecTemplate.apispectemplate.replaceAll(this.adapterName, this.instanceName);
      }
  ...
  clickBtn(spec) {
    if (spec['button'] == "Try it out")
      spec['button'] = "Cancel";
    else
      spec['button'] = "Try it out";
        ...
        for (let value in this.apispecTemplate.paths[keys][key]) {
          if (value == "responses") {
            ...
            for (let resp in this.apispecTemplate.paths[keys][key][value]) {
              let respObj = {}
              respObj["status"] = resp
              respObj["description"] = this.apispecTemplate.paths[keys][key][value][resp]["description"]
              respObj["content"] = this.apispecTemplate.paths[keys][key][value][resp]["content"]
            ...
            for (let i = 0; i < this.apispecTemplate.paths[keys][key][value].length; i++) {
              this.apispecTemplate.paths[keys][key][value][i].value = this.apispecTemplate.paths[keys][key][value][i].value?.replace("{datasource}", this.adapter?.alias).replace("{org}", sessionStorage.getItem("organization"))
            }
            ...
            if (pathObj["requestType"] == "POST" && value == "requestBody") {
              if (this.adapterApispecTemplate?.paths?.[keys]?.[key]?.[value]) {
                pathObj[value] = this.adapterApispecTemplate.paths[keys][key][value];
              }
            }
          }
        }
  ...
  getSchemas() {
    ...
    if (this.apispecTemplate.components) {
      for (let component in this.apispecTemplate.components.schemas) {
        if (component == "input") {
          this.schema[component]?.push(JSON.stringify(this.apispecTemplate.components.schemas.input.properties))
        }
        else {
          this.schema[component]?.push(JSON.stringify(this.apispecTemplate.components.schemas.output.properties))
        }
        this.value = JSON.stringify(this.apispecTemplate.components.schemas.input.properties)
      }
    }
  }
  ...
  addMethod(spec: any, formType: any) {
    if (formType == 'API') {
      this.formType = "API"
      this.modalService.openModal(spec, 'standard');
    } else {
          ...
          dialogRef.afterClosed().subscribe((result) => {
            if (result === "delete") {
              ...
              this.adapterServices.updateAdapterAPISpec(this.adapter).subscribe((res) => {
                this.adapterServices.messageNotificaionService('success', "Done!  Method Deleted Successfully");
                this.ngOnInit();
              }
              );
            }
  ...
  addRemoteMethod(spec) {
    ...
    if (this.adapterApispecTemplate.paths) {
      ...
      this.adapterServices.updateAdapterAPISpec(this.adapterReplica).subscribe((res) => {
        this.adapterServices.messageNotificaionService('success', "Done! Method Added Successfully");
        this.ngOnInit();
      });
    }
  }
  ...
  execute(spec, template?) {
    ...
    if (spec.requestType.toLowerCase() == "post") {
      ...
      if (spec.requestBody.content['multipart/form-data']) {
        ...
        this.adapterServices.callPostApiForMultipartFormData(url, this.formData, params, headers).subscribe(resp => {
          this.response = resp
        ...
        if(this.restProvider){
          this.adapterServices.callPostApiFromEndpointSwagger(url, spec.requestBody.value, params, headers).subscribe(resp => {
            this.response = resp.body;
            this.generateCURL(spec.requestType, resp.url, headers, spec.requestBody.value);
      ...
      this.adapterServices.callGetApi(this.serverUrl + this.specPath, params, headers).subscribe(resp => {
        this.response = resp
      ...
      this.adapterServices.callDeleteApi(this.serverUrl + this.specPath, params, headers).subscribe(resp => {
        this.response = resp
        this.generateCURL(spec.requestType, resp.url, headers, null);
      }, err => {
        this.response = err
  ...
  generateCURL(requestType: string, url: string, headers: any, requestBody: string) {
