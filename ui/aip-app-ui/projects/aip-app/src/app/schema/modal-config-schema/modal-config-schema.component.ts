  ...
  implements OnInit, OnChanges, AfterViewInit
{
            ...
            this.services.getSchemaFormsByName(name).subscribe(
              (resp) => {
                ...
                if(!this.isInView){
                  ...
                  this.capabilities=this.dropDownVauleCapbility.get(this.selectedtype).value;
                  this.selectedCapability=res.capability;
                }
               else{
                this.types=[{ value: res.type, viewValue: res.type }];
                this.selectedtype=res.type;
                this.capabilities=[{ value: res.capability, viewValue: res.capability }];
                this.selectedCapability=res.capability;
               }
  ...
  navigateToSchema() {
    if (this.checkUniqueIdColumn()) {
      ...
      this.services.message('Please add "uniqueId" field under "recordcolumnname" for Graph View.', 'info');
    }
  }
  ...
  onSelectionType(event: any) {
    console.log("values to be shown ", this.dropDownVauleCapbility.get(event).value, this.dropDownVauleCapbility.get(event).default)
    this.selectedCapability = this.dropDownVauleCapbility.get(event).default as string;
