...
@Component({
  ...
  styleUrls: ["./modal-config-schema-editor.component.css"],
})
  ...
  validateForm() {
    if (this.action != "Update") {
      if ((
        ...
        this.local_data.recordcolumndisplayname?.replace(/\s/g, "").length > 0
      ) && (this.propertieslist.filter(word => word == this.local_data.recordcolumnname).length == 0)
        && (this.displaynamelist.filter(word => word == this.local_data.recordcolumndisplayname).length == 0)) {
        this.disableSave = false;
      }
      else {
      ...
      for(let i=0;i<this.propertieslist.length;i++){
        ...
        if((this.local_data.recordcolumnname=="")||(this.local_data.recordcolumndisplayname=="")||(this.propertieslist[i]==this.local_data.recordcolumnname)||(this.displaynamelist[j]==this.local_data.recordcolumndisplayname)){
          this.disableSave=true;  
