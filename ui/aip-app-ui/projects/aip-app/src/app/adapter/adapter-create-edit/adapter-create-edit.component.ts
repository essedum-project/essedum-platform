...
export class AdapterCreateEditComponent {
  ...
  createAdapter() {
    if (this.data.executiontype == 'REMOTE') {
      this.data.apispec = this.specTemplates.filter(spec => spec.domainname == this.data.spectemplatedomainname)[0].apispectemplate;
  ...
  updateAdapter() {
    ...
    this.adapterServices.updateAdapter(this.data)
      .subscribe(res => {
  ...
  chainNameSelectChange(chainNameSelected:string){
    this.selecteddChain = this.chains.filter(chain => chain.alias == chainNameSelected)[0];
 ...
 resolvePromise(){
    ...
    this.adapterServices.getMlSpecTemplatesCards(this.org)
      .subscribe(res => {
        ...
        this.specTemplates.forEach((specTem) => {
          this.specTemplatesOptions.push(new OptionsDTO(specTem.domainname, specTem.domainname));
        });
      });
  }
  ...
  if(this.data.chainName){
    var reqChain = this.chains.filter(chain=> chain.name == this.data.chainName)[0];
