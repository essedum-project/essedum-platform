...
export class MethodCreateEditComponent {
 ...
 methodType:boolean=true; 
  ...
  transformScriptPython = 'def execute(connectionDetails,body):\n\n\t//start your code from here\n\n\treturn response';
  ...
  ngOnInit(): void {
    ...
    this.adapterOptions.push(new OptionsDTO(this.adapter.name, this.adapter.name));
  ...
  getDatasetForCreateMethod() {
    let attributes = { RequestMethod: this.data.requestType, Headers: [], QueryParams: [], PathVariables: [], Url: "" }
    let headersData = []
    let paramsData = []
    let pathVariables = []
    let datasetObj = {}
    if (this.data) {
      ...
      if (!this.data.parameters?.filter(par => par.name == "cloud_provider")[0]) {
    ...
    datasetObj["createdby"] = JSON.parse(sessionStorage.getItem('user')).user_email
  ...
  getDatasetForEditMethod(spec: any) {
    try {
      if (this.adapterApispecTemplate.paths) {
        if (this.adapterApispecTemplate.paths[spec.path] && this.adapterApispecTemplate.paths[spec.path][spec.requestType.toLowerCase()]) {
          let method = this.adapterApispecTemplate.paths[spec.path][spec.requestType.toLowerCase()];
          if (method.dataset) {
            this.adapterServices.getDataset(method.dataset).subscribe(res => {
              ...
              this.transformScriptPython=JSON.parse(this.dataset.attributes.transformScriptTransformationScript);
            ...
            this.dynamicRequirementsArray=this.dataset.attributes.requirements;
            this.isRemoteExecution = this.dataset.attributes.isRemoteExecution;
            this.remoteExecutorURL= this.dataset.attributes.remoteExecutorURL;
            this.transformScriptData= this.dataset.attributes.transformScriptData;
            this.selectedConnection = this.datasourcesForConnection.filter(datasource => datasource.alias ===  this.dataset.attributes.remoteConnectionAlias)[0];
          });
          }
        }
      }
  ...
  getDataSource() {
    this.adapterServices.getDataSource(this.adapter.connectionid)
      .subscribe(resp => {
        console.log(resp);
  ...
  proceedContent() {
    try {
      ...
      if (JSON.stringify(this.dataset.attributes) === JSON.stringify({})) {
        ...
        this.dynamicPathVariablesArray = this.dataset.attributes.PathVariables;
        this.dynamicConfigVariablesArray = this.dataset.attributes.ConfigVariables;

        if (this.dataset.attributes.bodyOption == "raw") {
          if (this.dataset.attributes.bodyType == "JSON" || this.dataset.attributes.bodyType == "FILE")
            this.schemaForm = JSON.parse(this.dataset.attributes.Body);
          else if (this.dataset.attributes.bodyType == "Text" || this.dataset.attributes.bodyType == "XML")
  ...
  findalldatasourcesForConnection() {
    this.adapterServices.getDatasources()
      .subscribe(res => {
        this.datasourcesForConnection = res;
        this.remotePromise=Promise.resolve(true);
        this.datasourcesForConnection = this.datasourcesForConnection.filter(datasource => (datasource.interfacetype == null || datasource.interfacetype != 'adapter') && datasource.organization == sessionStorage.getItem("organization") && (datasource.type == 'REMOTE'));
        this.datasourcesForConnection.forEach((datasource) => {
          this.connectionOptions.push(new OptionsDTO(datasource.type.concat('-' + datasource.alias), datasource.alias));
        });
  ...
  connectionNameSelectChange(connectionNameSelectd: string) {
    ...
    this.selectedConnection = this.datasourcesForConnection.filter(datasource => datasource.alias == connectionNameSelectd)[0];
   console.log(this.selectedConnection);
    this.dataset.attributes.remoteConnectionAlias = this.selectedConnection.alias;
    ...
    try {
      this.service.getDatasetJson()
        .subscribe(resp => {
          ...
          if (this.dataset.attributes && this.dataset.attributes.ScriptType == 'Python' && this.dataset.attributes.transformScriptTransformationScript != "")
            this.transformScriptPython = JSON.parse(this.dataset.attributes.transformScriptTransformationScript);
            this.transformScriptData = this.dataset.attributes?.transformScriptData;
  ...
  getdatasetTypes() {
    try {
      this.adapterServices.getDatasetJson()
        .subscribe(resp => {
          this.datasetTypes = resp;
          this.dataset.backingDataset = this.dataset.backingDataset &&
            this.dataset.backingDataset !== null ? this.dataset.backingDataset : '';
          if (this.connectionType) {
            this.sourceType = this.datasetTypes.filter(row =>
              row.type.toLocaleLowerCase() === this.connectionType.toLocaleLowerCase())[0];
          }
          ...
          if (this.dataset.attributes && this.dataset.attributes.preTransformationScript && this.dataset.attributes.preTransformationScript != "")
  ...
  addHeaderRowBasedOnType(type: string) {
    ...
    if (type === 'FILE') {
      const index = this.dynamicHeadersArray.findIndex(header => header.key.toLowerCase() === 'content-type');
      if (index !== -1) {
        this.dynamicHeadersArray.splice(index, 1);
      }
      return;
    ...
    const existingHeader = this.dynamicHeadersArray.find(header => header.key.toLowerCase() === 'content-type');
    if (existingHeader) {
      existingHeader.value = contentTypeValue;
    } else {
      this.dynamicHeadersArray.push({ key: 'Content-Type', value: contentTypeValue });
    }
  }
  ...
  testConnection() {
    ...
    if (this.formType == 'API' || !this.dataset.attributes["ScriptType"]) {
      ...
      this.adapterServices.testConnection(this.dataset).subscribe((response) => {
        this.adapterServices.messageNotificaionService('success', "Tested!  Connected Successfully");
      },
        error => {
          this.adapterServices.messageNotificaionService('warning', "Tested!  FAILED");
        });
    }else{
      if (this.bodyOption == "raw") {
  ...
  saveDataset() {
    ...
    if (this.formType == 'API' || !this.dataset.attributes["ScriptType"]) {
      ...
      if (this.dataset.attributes.ScriptType == 'Python'){
        ...
        var transformScriptPython1 = JSON.stringify(this.transformScriptPython).replace(/\s/g, "").replace(/[,]["]["]/g, "");
        var transformScriptPython2 = JSON.stringify(this.transformScriptPython_cpy).replace(/\s/g, "").replace(/[,]["]["]/g, "");

        if (transformScriptPython1 != transformScriptPython2)
          this.dataset.attributes.transformScriptTransformationScript = JSON.stringify(this.transformScriptPython);
        else
  ...
  modifyAPISpec(dataset, name, pathUrlWithOrg, specParams) {
    ...
    if (this.apiSpecServerUrl && this.apiSpecServerUrl.includes("api")) {
      const path = this.apiSpecServerUrl.replaceAll(this.adapter.name, "{spec}").replaceAll(sessionStorage.getItem("organization"), "{org}").replaceAll(window.location.origin, "{host}");
    ...
    if (this.adapter.apispec != "{}") {
      ...
      if (apispec.paths[pathUrl])
        apispec.paths[pathUrl][attributes.RequestMethod.toLowerCase()] = path[pathUrl][attributes.RequestMethod.toLowerCase()];
      else {
  ...
  handleFileInput(files: any) {
    ...
    this.adapterServices.uploadFileToServerForAdapterMethod(this.formData, this.dataset.adaptername, this.dataset.alias).subscribe(res => {
      if (res.body) {
        this.schemaForm = res.body;
        if (this.schemaForm)
          this.schemaForm['isForTest'] = true;
      }
    });
  }
