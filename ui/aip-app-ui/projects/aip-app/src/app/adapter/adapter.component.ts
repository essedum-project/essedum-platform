...
export class AdapterComponent implements OnInit, OnChanges{
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (params['page']) {
        ...
        this.selectedSpecList = params['specList']
          ...
          : [];
        this.selectedConnectionNamesList = params['connectionList'] ? params['connectionList'].split(',') : [];
        this.selectedCategoryList = params['categoryList']
          ? params['categoryList'].split(',')
          : [];
        if ((this.selectedSpecList && this.selectedSpecList.length > 0) ||
  ...
  updateQueryParam(
    ...
    roleId: string | null = JSON.parse(sessionStorage.getItem('role') || '{}').id
  ) {
  ...
  filterSelectedCards(page:any,size?:any) {
    this.tagrefresh = false;
    if (this.selectedConnectionNamesList.length > 0 || this.selectedCategoryList.length > 0 || this.selectedSpecList.length > 0) {
  ...
  tagSelectedEvent(event){
    this.pageNumber=1;
     this.selectedConnectionNamesList=event.getSelectedMlAdapterConnectionType();
