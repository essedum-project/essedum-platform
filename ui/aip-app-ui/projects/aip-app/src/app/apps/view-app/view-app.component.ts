import { AfterViewInit, ChangeDetectorRef, Component, ElementRef, Input, OnChanges, OnInit, SimpleChanges, ViewChild, ViewContainerRef } from '@angular/core';
...
import { ParamArgumentPopupComponent } from '../../pipeline.description/param-argument-popup/param-argument-popup.component';
...
export class ViewAppComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  @ViewChild('loadAppContainer', { read: ViewContainerRef }) loadAppContainer!: ViewContainerRef;
  pdfUrl: string;
  ...
  ngOnInit(): void {
    ...
    this.iFrameUrl = this.sanitizer.bypassSecurityTrustResourceUrl(this.apptryoutlink);
      if (this.appType == 'video' || this.appType == 'pipeline' || this.appType == 'chain') {
        ...
        this.isMfe = true;
        this.safeMfeUrl = this.sanitizer.bypassSecurityTrustResourceUrl(resp['tryoutlink']);
        this.isrunApp = false;
        this.isOverview = false;
        this.switchtoVideoView = false;
        this.mfeApp = resp['mfeAppName']
        this.componentMfe = this.remoteLoad.navigateToRemoteComponent(this.mfeApp, resp['tryoutlink']);
      ...
      this.service.getStreamingServicesByName(this.appname, resp['organization']).subscribe(respPipeline => {
      ...
      if (this.appData.scope == 'video') {
        this.service.getDataset(this.appData.jobName).subscribe(resp => {
          ...
          this.service.getDatasource(this.dataset.datasource).subscribe(resp => {
            ...
            this.service.getProxyDbDatasetDetails(
              ...
              if (resp.length === 0) {
                this.service.message('There is an application error, please contact the application admin', 'error');
              } else {
                this.datasetData = resp;
                let obj = JSON.parse(this.dataset.attributes);
                this.fileName = obj.path  + '/' + obj.object;
                this.isVideo = true
                this.service.getStreamingServicesByName(this.appname).subscribe(res => {
                  this.streamItem = res
                  let json_content = JSON.parse(this.streamItem.json_content)
                  this.configData = json_content['elements'][0]['attributes']['arguments']
        ...
        this.service.readScriptFile(this.appname, "filename").subscribe(resp => {
  ...
  constructor(private router: Router, private route: ActivatedRoute,
    public sanitizer: DomSanitizer, private ledsLibService: LedsLibService,
    private service: Services, public dialog: MatDialog,
  ...
  runPipeline() {
    this.service.runPipeline(this.streamItem.alias ? this.streamItem.alias : this.streamItem.name, this.streamItem.name, 'NativeScript', this.selectedRunType['type'], this.selectedRunType['dsName']).subscribe(
      pageResponse => {
  ...
  saveScript() {
    ...
    this.service.saveNativeScript(this.appname, 'python3', formData).subscribe(resp => {
      console.log(resp)
    })
    let json_content = JSON.parse(this.streamItem.json_content)
  ...
  redirectToPipeline() {
    this.service.getStreamingServicesByName(this.appData.jobName).subscribe((res) => {
      this.streamItem = res
      ...
      if (this.streamItem.type === 'NativeScript') {
        this.router.navigate(['../../pipelines/view' + '/' + this.appData.jobName], navigationExtras);
      } else {
        this.router.navigate(['../../pipelines/view/drgndrp' + '/' + this.appData.jobName], navigationExtras);
      }
    });
  ...
  redirectToChain() {
    this.service.getStreamingServicesByName(this.appData.jobName).subscribe((res) => {
      ...
      if (this.streamItem.type === 'NativeScript') {
        this.router.navigate(['../../chain-list/view' + '/' + this.appData.jobName], navigationExtras);
      } else {
        this.router.navigate(['../../chain-list/view/drgndrp' + '/' + this.appData.jobName], navigationExtras);
      }
    });
  }
