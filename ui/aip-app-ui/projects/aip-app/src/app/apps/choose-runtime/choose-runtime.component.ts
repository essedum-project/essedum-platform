...
export class ChooseRuntimeComponent implements OnInit{
  ...
  ngOnInit() {  
    this.service.fetchJobRunTypes().subscribe(resp=>{
      ...
      if(this.data.default_runtime){
        ...
        let index = this.runTypes.findIndex(x => JSON.stringify(x.value) === this.data.default_runtime);
        if(index > -1){
  ...
  runtimeChange($event) {
    ...
    let index = this.runTypes.findIndex(option => option.viewValue === this.selectedRunType);
    this.selectedRunType = this.runTypes[index].value
  }
}
