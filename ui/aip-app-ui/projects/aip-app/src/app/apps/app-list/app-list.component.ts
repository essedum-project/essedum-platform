import { Component, DebugElement, ElementRef, EventEmitter, HostListener, OnInit } from '@angular/core';
...
export class AppListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (params['page']) {
        ...
        this.selectedTagType = params['type']
          ? params['type'].split(',')
          : [];
          this.selectedTag = params['tagId']
          ? params['tagId'].split(',')
          : [];
    ...
    this.service.getConstantByKey(this.appConstantsKey).subscribe((response) => {
  ...
  getAllApps(page?: any, size?: any,filter?:any) {
    ...
    this.updateQueryParam(
      this.pageNumber,
      this.filter,this.selectedTagType.toString(),this.selectedTag.toString()
    );
  ...
  deleteApp(index) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === "delete") {
        this.service.deleteStreamingService(this.appData[index]["cid"]).subscribe(resp => {
          this.getAllApps()
        })
        this.service.getAppByName(this.appData[index]["name"]).subscribe(resp => {
  ...
  searchApp() {
    ...
    this.updateQueryParam(this.pageNumber,this.filter,this.selectedTagType.toString(),this.selectedTag.toString());
  }
  getCountPipelines() {
    let params: HttpParams = new HttpParams();
    if (this.filter.length >= 1) params = params.set('query', this.filter);
  ...
  startApp(app) {
    this.app = app;
    this.service.getAppByName(app.name).subscribe(resp => {
      this.service.getStreamingServicesByName(resp.jobName, resp.organization).subscribe(
        (response) => {
          this.streamItem = response
          let json = JSON.parse(this.streamItem.json_content);
          if(this.app.scope == "Apps" && json.hasOwnProperty('default_runtime')){
  ...
  runScript() {
    ...
    // passType = this.type
    this.service.runPipeline(this.streamItem.alias ? this.streamItem.alias : this.streamItem.name, this.streamItem.name, passType, this.selectedRuntime.type, this.selectedRuntime.dsName, "generated").subscribe(
      res => {
        this.service.message('Pipeline has been Started!', 'success');
        let job_id = JSON.parse(res).jobId
        job_id = job_id.replaceAll("-", "")
        this.service.getStreamingServicesByName(this.app.name, this.app.organization).subscribe(
          (response) => {
  ...
  getLatestJobId(app) {
    ...
    if (json.latest_jobid)
      return json.latest_jobid
    else
      return false
  }
  ...
  runNativeScript(pipeline) {
    this.service.runPipeline(pipeline.alias ? pipeline.alias : pipeline.name, pipeline.name, 'NativeScript', this.selectedRuntime['type'], this.selectedRuntime['dsName']).subscribe(
      res => {
        this.service.message('Pipeline has been Started!', 'success');
        let job_id = JSON.parse(res).jobId
        job_id = job_id.replaceAll("-", "")
  ...
  redirectToPipeline(jobName) {
    this.service.getStreamingServicesByName(jobName).subscribe((res) => {
      ...
      if (this.streamItem.type === 'NativeScript') {
        this.router.navigate(['../pipelines/view' + '/' + jobName], navigationExtras);
      } else {
        this.router.navigate(['../pipelines/view/drgndrp' + '/' + jobName], navigationExtras);
      }
    });
  ...
  redirectToChain(jobName) {
    this.service.getStreamingServicesByName(jobName).subscribe((res) => {
      ...
      if (this.streamItem.type === 'NativeScript') {
        this.router.navigate(['../chain-list/view' + '/' + jobName], navigationExtras);
      } else {
        this.router.navigate(['../chain-list/view/drgndrp' + '/' + jobName], navigationExtras);
      }
    });
  }
