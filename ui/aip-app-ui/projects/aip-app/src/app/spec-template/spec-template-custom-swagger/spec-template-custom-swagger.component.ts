...
export class SpecTemplateCustomSwaggerComponent implements OnInit {
  ...
  getSpecTemplate(spectemplatedomainname: string) {
    this.adapterServices.fetchApiSpecTemplate(spectemplatedomainname,sessionStorage.getItem("organization")).subscribe((res) => {
      ...
      this.mlspecTemplate.apispectemplate=this.mlspecTemplate.apispectemplate.replaceAll("{datasource}", this.data.name).replaceAll("{org}", sessionStorage.getItem("organization")).replaceAll("{spec}", spectemplatedomainname).replaceAll("{host}", this.serverUrl);
      if(this.mlspecTemplate.apispectemplate && this.mlspecTemplate.apispectemplate.includes("adapters/v1")){
        ...
        for (let value in this.apispecTemplate.paths[keys][key]) {
          if (value == "responses") {
            ...
            for (let i = 0; i < this.apispecTemplate.paths[keys][key][value].length; i++) {
              this.apispecTemplate.paths[keys][key][value][i].value = this.apispecTemplate.paths[keys][key][value][i].value?.replace("{datasource}", this.data?.alias).replace("{org}", sessionStorage.getItem("organization"))
            }
  ...
  execute(spec, template?) {
    ...
    if (spec.parameters) {
      for (let param of spec.parameters) {
        if (param.in == "params" || param.in == "query") {
          params[param.name] = param.value ? param.value.replace("{datasource}", this.data).replace("{org}", sessionStorage.getItem("organization")) : ""
          if (!param.value)
            param.value = ""
        }
        if (param.in == "header") {
    ...
    if (spec.requestType.toLowerCase() == "post") {
      ...
      this.adapterServices.callGetApi(this.serverUrl + spec.path, params, headers).subscribe(resp => {
        this.response = resp
      }, err => {
        this.response = err
        if (!err) this.response = "ERROR"
      ...
      this.adapterServices.callDeleteApi(this.serverUrl + spec.path).subscribe(resp => {
        this.response = resp
      }, err => {
        this.response = err
        if (!err) this.response = "ERROR"
