...
export class CreateSpecTemplateComponent implements OnInit {
  ...
  capabilityList = [{viewValue:"Dataset",value:"dataset"},
  ...
  nameValidator: any;
  constructor(private route: ActivatedRoute, private dsService: Services,
    private location: Location,
    private telemetry: OpenTelemetryService,
    private router: Router, private service: AdapterServices) {
  ...
  authentications() {
    this.dsService.getPermission("cip").subscribe(
      (cipAuthority) => {
  ...
  domainNameChanges(specName:string) {
    ...
    if (this.regexPatternObj.test(specName)) {
      ...
      } else if (specName.match(this.regexPatternForValidAlphabetsObj) == null) {
