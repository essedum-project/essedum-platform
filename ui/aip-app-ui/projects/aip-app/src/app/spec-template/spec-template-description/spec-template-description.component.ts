...
export class SpecTemplateDescriptionComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.editorOptions.onChange = () => {
      ...
      this.adapterService.fetchApiSpecTemplate(this.route.snapshot.params.cname, sessionStorage.getItem("organization")).subscribe((resp: any) => {
        this.card = resp;
        if (this.card && this.card.apispectemplate && this.card.apispectemplate.includes("adapters/v1")) {
          this.executionRequired = true;
        }
        let timezoneOffset = new Date().getTimezoneOffset();
        this.card.createdon = new Date(new Date(this.card.createdon).getTime() - timezoneOffset * 60 * 1000);
      });
    
  }

  findRelatedAdapters(dName) {
    this.adapterService.getAdaptersBySpectempdomainname(dName).subscribe((resp: any) => {
      resp.forEach((res) => {
  ...
  findRelatedInstances(dName) {
    this.adapterService.getInstancesBySpectempdomainname(dName).subscribe((resp: any) => {
  ...
  redirectToSelectedComponent(component) {
      if (component.viewValue == 'Instance')
        this.router.navigate(["../../instances/" + component.value], { relativeTo: this.route })
      else if (component.viewValue == 'Spec')
        this.router.navigate(["../../specs/" + component.value], { relativeTo: this.route })
      else
        this.router.navigate(["../../implementations/" + component.value], { relativeTo: this.route })
  ...
  deleteSpecTemplate(domainName: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result === "delete") {
        this.adapterService.deleteApiSpecTemplate(domainName, sessionStorage.getItem("organization")).subscribe((res) => {
          this.adapterService.messageService(res, "Spec Deleted Successfully");
          this.toggler();
        }, error => { this.adapterService.messageService(error); });
      }
    });
  }
  listToggle() {
      ...
      if (respAdp) {
        respAdp.forEach(adpName => {
          if (!this.relatedAdaptersList.includes(adpName))
            this.nonrelatedAdapters.push(new OptionsDTO('Implementation', adpName));
        });
      }
    });
  }
  ...
  fethchAllSpecTemplateNamesByOrg() {
    this.adapterService.getSpecTemplateNamesByOrganization().subscribe((respSpec) => {
      if (respSpec) {
  ...
  fethchAllInstanceNamesByOrg() {
    this.adapterService.getMlInstanceNamesByOrganization().subscribe((respMlIns: any) => {
  ...
  setDefault(component) {
    this.adapterService.changePriorityBySpecTemDomNameAndInstancNameAndOrg(this.domainName, component.value, 1, sessionStorage.getItem("organization")).subscribe((resp: any) => {
      if (resp && typeof resp === 'object') {
        this.ngOnInit();
      }
    });
  ...
  changePriority(event, component) {
    this.adapterService.changePriorityBySpecTemDomNameAndInstancNameAndOrg(this.domainName, component.value, event.target.value, sessionStorage.getItem("organization")).subscribe((resp: any) => {
      if (resp && typeof resp === 'object') {
        this.ngOnInit();
      }
    });
