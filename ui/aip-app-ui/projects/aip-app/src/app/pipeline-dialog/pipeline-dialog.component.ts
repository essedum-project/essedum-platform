...
export class PipelineDialogComponent {
  ...
  errCount: number=0;
    ...
    constructor(
      ...
      private service: Services, private jobService: JobsService,
  ...
  getCardStatus(){
    ...
    this.filteredTopics.forEach(e=>{
      this.jobService.getByCorelationId(e.corelId).subscribe(res=>{
        ...
        // const uniqueEvent = {...e, status: res[0].jobStatus};
        // if (!this.cardData.some(card => card.eventName === uniqueEvent.eventName)) {
        //   this.cardData.push(uniqueEvent);
        //   this.searchCards.push(uniqueEvent);
        // }
      })
    });
  }
