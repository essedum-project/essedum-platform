...
resultAsJson=$(curl -u ${ARTIFACTORY_REPO_USER}:${ARTIFACTORY_REPO_PASS} -X POST  https://infyartifactory.jfrog.io/artifactory/api/search/aql -H "content-type: text/plain" -d 'items.find({ "repo": {"$eq":"icets-ai_npm_virtual"}, "name": {"$match" : "'"$moduleName"-'ui-'"$releaseVersion"'.*.tgz"}}).sort({"$desc":["modified","created"]}).limit(5)')
